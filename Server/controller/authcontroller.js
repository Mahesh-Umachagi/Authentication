...
router.get('/projects/:id',(request,response)=>{
    ...
    authmodel.fetchProjectDetails(id,(result)=>{
        response.status(200).send(result);
    })
})
...
router.put('/projects/:id',(request,response)=>{
    ...
    review.ratings = parseInt(review.ratings);
    authmodel.putReview(id,review,(result)=>{
        response.status(200).send(result);
    })

})

router.get('/premiumprojects',(request,response)=>{
    authmodel.fetchPremiumProjects((result)=>{
        response.status(200).send(result);
    })
})
